...
class Generator:
    ...
    def load_model(self):
        ...
        # init bpe
        from . bpe_tokenizer import HuggingFaceByteLevelBPE
        self.bpe = HuggingFaceByteLevelBPE()
    ...
    def generate(
        ...
        for i in range(ori_bs):
            ...
            for tokens in output_ids[i]:
                ...
                result = {}
                result["text"] = text
                result["tokens"] = [self.bpe.bpe.decode([t]) for t in tokens]
                result["text_offset"] = token_offsets
