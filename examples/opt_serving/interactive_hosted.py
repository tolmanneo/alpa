...
from opt_serving.service.constants import (
    ...
    MAX_BS, NUM_BEAMS, MAX_BEAM, NUM_RETURN_SEQ,
...
@app.route("/completions2", methods=["POST"])
def completions2():
    ...
    if "stop" in generation_args:
        ...
        if stop is None:
            ...
            stop = [generator.encode(stop)[0] for s in stop]
    ...
    cur_len = max(len(p) for p in prompts) # TODO (neo): may disable this
    check_max_length_limit(cur_len, MAX_SEQ_LEN) # TODO (neo): may disable this
    ...
    for i, prompt in enumerate(prompts):
        ...
        generate_batch_queue.put(
            WorkItem(cost=len(prompt) + max_len, 
                     uid=i, 
...
def completions():
    ...
    # results_logprobs = ret_queue.get()
    # for k, v in enumerate(results_logprobs[1]['logprobs']):
    #     results[k]['token_scores'] = v
